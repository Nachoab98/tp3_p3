	...
	public static int maxGanancia(LinkedList<Reserva> ofertas){
		...
		_ofertas.sort(new Comparator<Reserva>() {
			...
			public int compare(Reserva o1, Reserva o2) {
				int res = Integer.compare(o1.fin(), o2.fin());
				if (res != 0) {
					return res;
				}
				res = Integer.compare(o1.inicio(), o2.inicio());
				if (res != 0) {
					return res;
				}
		...
		for(int i = 0; i < _ofertas.size(); i++) {
			_wTable[i] = _ofertas.get(i).precio();
		}
		...
		for (int i = 0; i < _ofertas.size(); i++) {
			_pTable[i] = -1;
			for (int j = 0; j < i; j++) {
				if (_ofertas.get(j).fin() <= _ofertas.get(i).inicio()) {
					_pTable[i] = j;
				}
			}
		}
